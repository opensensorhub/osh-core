 ...
 * @since Jun 18, 2025
 */
public class DataStreamIdEncoder implements IdEncoder
{
    final IdEncoder delegate;
    final CurieResolver curieResolver;
    final IDataStreamStore dsStore;
    
    
    public DataStreamIdEncoder(IdEncoder delegate, CurieResolver curieResolver, IDataStreamStore dsStore)
    {
        ...
        this.dsStore = Asserts.checkNotNull(dsStore, "dsStore");
    }
    ...
    public BigId decodeID(String encodedID)
    {
        ...
        // datastream CURIEs must be of the form [prefix:sysName:outputName]
        if (encodedID.contains(":")) {
            ...
            var outputName = uri.substring(lastSep+1);
            var k = dsStore.getLatestVersionKey(sysUri, outputName);
            if (k == null)
                throw new IllegalArgumentException("Unknown ID: " + encodedID);
